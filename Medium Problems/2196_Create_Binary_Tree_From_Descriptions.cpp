 ...
 */
 /*
    APPROACH:
    1. We can use a hash map to store the parent-child relationships and a hash set to keep track of all the children nodes.
    2. We iterate through the descriptions and populate the hash map and hash set.
    3. After processing all descriptions, we can identify the root node as the one that is not present in the hash set of children.
    4. Finally, we return the root node from the hash map.
